...
namespace JKress.AITrainer
{
    public class WalkerAgent : Agent
    {
        ...
        [Range(0.1f, 6)] [SerializeField] float m_TargetWalkingSpeed = 2; 
        float m_minWalkingSpeed = 0.1f; 
        float m_maxWalkingSpeed = 6; 
        ...
        public override void OnEpisodeBegin()
        {
            ...
            spine.rotation = Quaternion.Euler(0, Random.Range(0.0f, 360.0f), 0);
        ...
        public void CollectObservationBodyPart(BodyPart bp, VectorSensor sensor)
        {
            ...
            //sensor.AddObservation(m_OrientationCube.transform.InverseTransformDirection(bp.rb.position - hip.position));
            sensor.AddObservation(m_OrientationCube.transform.InverseTransformDirection(bp.rb.position - spine.position));
            ...
            if (bp.rb.transform != spine) sensor.AddObservation(bp.currentStrength / m_JdController.maxJointForceLimit);
        }
        ...
        public override void CollectObservations(VectorSensor sensor)
        {
            ...
            //rotation deltas
            sensor.AddObservation(Quaternion.FromToRotation(spine.forward, cubeForward));
        ...
        void UpdateOrientationObjects()
        {
            ...
            m_OrientationCube.UpdateOrientation(spine, targetT);
        }
        ...
        /*
        // returns the velocity of armL+legR or armR+legL to force the velocity into a certain range
        Vector3 GetArmLegVelocity()
        {
            //Vector3 ArmLegvelsum = Vector3.zero;
            Vector3 ArmLvelsum = Vector3.zero;
            Vector3 ArmRvelsum = Vector3.zero;
            Vector3 LegLvelsum = Vector3.zero;
            Vector3 LegRvelsum = Vector3.zero;
            Vector3 ArmRLegLvelsum = Vector3.zero;
            Vector3 ArmLLegRvelsum = Vector3.zero;

            // add rbs
            foreach (var item in m_JdController.bodyPartsList)
            {
                if (item == m_JdController.bodyPartsDict[armL]) { ArmLvelsum += item.rb.velocity;}
                else if (item == m_JdController.bodyPartsDict[forearmL]) { ArmLvelsum += item.rb.velocity;}
                else if (item == m_JdController.bodyPartsDict[armR]) { ArmRvelsum += item.rb.velocity;}
                else if (item == m_JdController.bodyPartsDict[forearmR]) { ArmRvelsum += item.rb.velocity;}
                else if (item == m_JdController.bodyPartsDict[thighL]) { LegLvelsum += item.rb.velocity;}
                else if (item == m_JdController.bodyPartsDict[shinL]) { LegLvelsum += item.rb.velocity;}
                else if (item == m_JdController.bodyPartsDict[thighR]) { LegRvelsum += item.rb.velocity;}
                else if (item == m_JdController.bodyPartsDict[shinR]) { LegRvelsum += item.rb.velocity;}
            }

            ArmRLegLvelsum = ArmLvelsum + LegRvelsum;
            ArmLLegRvelsum = ArmRvelsum + LegLvelsum;

            var avgRLVel = (ArmRLegLvelsum + ArmLLegRvelsum) / 2;
            return avgRLVel;
        ...
        */
        ...
        public float GetMatchingVelocityReward(Vector3 velocityGoal, Vector3 actualVelocity)
        {
            ...
            //fix nan error
            if (MTargetWalkingSpeed == 0) MTargetWalkingSpeed = 0.1f;
