...
public:
	...
	void CreateTrackUsingCubes(string textureFilename);
	void RenderTrackUsingCubes();
	...
	glm::vec3 MoveAlongCenterline(const glm::vec3& currentPosition, float distance);
	glm::mat4 ChangeOrientationOfModel(const glm::vec3& currentPosition, float distance);
...
private:
	...
	GLuint vao_innerCurve;
	GLuint vao_outerCurve;
	GLuint m_vaoTrack;
	...
	vector<glm::vec3> inner_curve;	// 
	vector<glm::vec3> outer_curve;	// 
