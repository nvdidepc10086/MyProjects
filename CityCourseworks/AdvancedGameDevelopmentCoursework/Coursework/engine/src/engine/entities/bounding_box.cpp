...
void engine::bounding_box::on_render(float r, float g, float b, const engine::ref<engine::shader>& shader, bool render)
{
	if (!render) return; // Do not render if render* is false
...
void engine::bounding_box::set_render(bool render)
{
	m_render = render;
}
